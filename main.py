...
def get_grad(pos_samples, neg_samles, word_embedding, context_embedding):
    word = pos_samples[0]
    context = pos_samples[1]

    word_grad = [(1-sigmoid(np.dot(word_embedding[word], context_embedding[c])))*word_embedding[word] for c in context]
    context_grad = {}
    [context_grad[c].add((1-sigmoid(np.dot(word_embedding[word], context_embedding[c])))*context_embedding[c]) for c in context]
    neg_context_grad = {}
    [neg_context_grad[c].add(-1*(1-sigmoid(-1*np.dot(word_embedding[word], context_embedding[c])))*context_embedding) for c in neg_samles]
